...
export function trackRecievedTwitchWebhook(messageId, timestamp) {
  // TODO: make sure that the ID is unique, so this insert will fail
  // if we are trying to count a message twice.
  return insert("twitch_webhook_log", {
    id: messageId,
    received_at: timestamp,
  });
}

export function addTwitchUserEvent(userId, streamId, data) {
  return insert("twitch_user_events", {
    ...data,
    user_id: userId,
    stream_id: streamId,
...
export async function createNewRewardsForUser(user, data) {
  ...
  const { error, data: record } = await until(async () =>
    base
      .from("user_rewards")
      .insert([{ ...data, user_id: user.id?user.id:user, updated_at: new Date().toISOString()}], { returning: "representation" })
      .single()
  );
...
export async function updateRewardForUser(user, data) {
  ...
  const { error, data: record } = await until(async () =>
    base
      .from("user_rewards")
      .update([{ ...data, user_id: user.id?user.id:user, updated_at: new Date().toISOString()}], { returning: "representation" })
...
export async function updateRewardActiveTrue(user, data) {
  ...
  const { error, data: record } = await until(async () =>
    base
      .from("user_rewards")
      .update([{ active: true, updated_at: new Date().toISOString() }], { returning: "representation" })
      .eq("user_id", user.id?user.id:user)
      .in("id", data)
      .select()
  );
...
export async function updateRewardActiveFalse(user, data) {
  ...
  const { error, data: record } = await until(async () =>
    base
      .from("user_rewards")
      .update([{ active: false, updated_at: new Date().toISOString() }], { returning: "representation" })
      .eq("user_id", user.id?user.id:user)
      .not("id", "in", `(${data})`)
      .select()
  );
